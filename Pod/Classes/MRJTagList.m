...
- (void)display {
    ...
    for (UIView *subview in [self subviews]) {
        if ([subview isKindOfClass:[MRJTagView class]]) {
            MRJTagView *tagView = (MRJTagView *)subview;
...
- (UIColor *)getSelectBackColor {
    if (!_selectBackColor) {
        _selectBackColor = [UIColor grayColor];
    }
...
- (void)setIsManySelect:(BOOL)isManySelect {
    _isManySelect = isManySelect;
}
...
#pragma mark - MRJTagViewDelegate
